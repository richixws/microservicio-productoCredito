...
public class ProductoCreditoApplication implements CommandLineRunner {
	...
	/*
	 * @Autowired private IBankCreditRepository bankCreditRespository;
	 * 
	 * @Autowired private IBankCreditTypeRepository bankCreditTypeRepository;
	 */
	...
	public void run(String... args) throws Exception {
		 ...
		 * .flatMap(tip -> bankCreditTypeRepository.save(tip)) .thenMany(Flux .just(new
		 * bankCredit("credito", 1,bankcredittype1), new bankCredit("credito", 1,
		 * bankcredittype2), new bankCredit("credito", 2, bankcredittype3), new
		 * bankCredit("credito", 2, bankcredittype4)) .flatMap(bankcredit ->
		 * bankCreditRespository.save(bankcredit))) .subscribe();
		 */
	}
}
